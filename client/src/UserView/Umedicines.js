...
import { Container, Row, Col, Card, CardBody, Button, Modal, ModalHeader, ModalBody, ModalFooter, Form, FormGroup, Label, Input, Alert, Table } from 'reactstrap';
...
const Medicines = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get(`http://localhost:5268/api/category`);
  ...
  const handleAddMedicine = async () => {
    try {
      ...
      setIsLoading(true);
      try {
        const response = await axios.post(`http://localhost:5268/api/Medicine`, formData);
